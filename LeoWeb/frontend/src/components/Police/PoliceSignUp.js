...
/*
 * Class to implement sign up functionality for Police
 * Note - not to be shown on universal web app
 */
export default class PoliceSignUp extends React.Component {
	constructor(props) {
		super(props)
		this.state = {
			name: "",
	...
	onSubmit = async e => {
		...
		if (this.state.password === this.state.confirmpassword) {
			...
			var res;
			await axios.post('https://peaceful-refuge-01419.herokuapp.com/LeoHelp/addPolice', data)
				.then(response => {
					res = response.status;
				})
				.catch(error => {
			...
			<div>
				...
				<center>
					<form onSubmit={this.handleSubmit}>
						...
						<Link to="/PoliceSignIn">SignIn </Link>
