            ...
            for i, link in enumerate(articles_lst):
                ...
                if i == 0:
                    ...
                    for crime_ in crime_list:
                        if (
                            crime_[0].lower() == crime.lower()
                            or crime_[1].lower() == crime.lower()
                        ):
            ...
            for ent in doc.ents:
                if ent.label_ == "GPE":
                    ...
                    if ent.text.lower() in cities_lst and city == "":
                        ...
                        ent.text.lower() not in cities_lst
                        and ent.text.lower() not in extra_locs
                        and check_in_dict(ent.text) == False
                    ):
        ...
        if len(found_loc) != 0:
            ...
            for token in about_doc:
                if token.tag_ == "NNP":
                    ...
                    if (
                        check_in_dict(token) == False
                        and str(token).lower() not in extra_locs
                    ):
                        words.append(token)
            # print(words)
            if len(words) == 0:
...
def get_date_and_headline(df) :
    ...
    for _, row in df.iterrows():
        ...
        if(date == None) :
            date = datetime.now()
        date_lst.append(date)
        headlines_lst.append(article.title)
        #print(type(article.publish_date))
    df["date"] = date_lst
