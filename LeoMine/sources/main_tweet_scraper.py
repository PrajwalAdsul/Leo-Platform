...
def main(argv):
    ...
    try:
        ...
        outputFileName = "./database/output_got.csv"
        ...
        if usernames:
            if len(usernames) > 1:
                ...
                if len(usernames) > 20 and tweetCriteria.maxTweets > 0:
                    maxtweets_ = (
                        len(usernames) // 20 + (len(usernames) % 20 > 0)
                    ) * tweetCriteria.maxTweets
                    print(
                        "Warning: due to multiple username batches `maxtweets' set to %i"
                        % maxtweets_
                    )
            else:
...
def MainTweetScrapper():
    main(["--username", "ani", "--toptweets", "--maxtweets", "100"])
