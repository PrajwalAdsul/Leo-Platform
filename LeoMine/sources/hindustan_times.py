...
def get_all_content(objects):
    ...
    def get_content(url):
        ...
        if response.status_code == 200:
            ...
            contents = html_content.find("div", {"class": "story-details"})
            if not contents:
                return "NA"
...
def get_headline_details(obj):
    try:
        ...
        timestamp_tag = obj.parent.parent.find("span", {"class": "time-dt"})
        if timestamp_tag is None:
            timestamp = datetime.now()
        else:
            content = timestamp_tag.contents[0].strip()
            timestamp = datetime.strptime(content, "%b %d, %Y %H:%M")
        return {
            "content": "NA",
...
def get_chronological_headlines(url):
    ...
    if response.status_code == 200:
        ...
        a_tags = list(
            map(lambda x: x.find("a"), soup.find_all("div", {"class": "media-body"}))
        )
...
def get_trending_headlines(url):
    ...
    if response.status_code == 200:
        ...
        a_tags = soup.find(
            "div", {"class": "news-area newtop-block mb-5 mt-10"}
        ).find_all("a")
        headlines = remove_duplicate_entries(
            map(get_headline_details, a_tags), "link", "title"
        )
        return headlines
    return None
