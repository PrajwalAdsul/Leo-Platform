    ...
    await fs.readdir("uploads/", function(err, files) {
      ...
      files.forEach(function(file) {
        ...
        if (!(file[0] == "d" && file[1] == "o" && file[2] == "c")) {
          fs.rename(
            "uploads/" + file,
            "uploads/doc" + req.body.user_name + "username" + req.body.name,
            function(err) {
    ...
    await fs.readdir("uploads/", function(err, files) {
      ...
      for (var i = 0; i < files.length; i++) {
        if (files[i][0] == "d" && files[i][1] == "o" && files[i][2] == "c") {
          if (
            files[i].split("doc")[1].split("username")[0] == req.body.user_name
          ) {
            try {
...
LeoHelp.route("/loginIn").post(async (req, res) => {
  try {
    ...
    console.log(data);
    if (data !== null) {
      // Changed: Requiring user data after login
...
LeoHelp.route("/addUser").post(function(req, res) {
  try {
    if (
      ...
      cr.save()
        .then(cr => {
          // Changed: Requiring user data after registering
...
LeoHelp.route("/markTrouble").put(async (req, res) => {
  ...
  user.findByIdAndUpdate(
    ...
    (error, data) => {
      //Changed: Requiring user data after marking the trouble
      if (error) {
      ...
      .then(json => {
        ...
        cr.save()
          .then(cr => {
            res
              .status(200)
              .json({ "message ": cr.name + " added successfully" });
          })
          .catch(err => {
            res.status(400).json({ Error: "Error adding new user" });
          });
...
LeoHelp.route("/deleteUser").delete(async (req, res) => {
  let data = await user.findOne({ user_name: req.body.user_name });
  if (data == null) res.status(400).json({ Error: "No such username" });
  // console.log("**********\n" + data + "\n*************\n");
...
LeoHelp.route("/getDoctor").post(async (req, res) => {
  let data = await doctor.findOne({ user_name: req.body.user_name });
  if (data == null) res.status(400).json({ msg: "error" });
  res.status(200).json(data);
});
